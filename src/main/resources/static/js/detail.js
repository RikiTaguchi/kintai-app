...
// 要素の取得（給与詳細）
const textDetail = document.getElementsByClassName("detail-text");
...
for (let i = 0; i < textDetail.length; i++) {
    textDetail[i].style.display = styleDisplay;
}
...
// 切替（給与詳細）
buttonDetail[0].addEventListener("click", () => {
    ...
    for (let i = 0; i < textDetail.length; i++) {
        if (url.searchParams.get("detail") === "on") {
            ...
            textDetail[i].style.display = "table";
        }
    }
    for (let i = 0; i < tableDetail.length; i++) {
        if (url.searchParams.get("detail") === "on") {
